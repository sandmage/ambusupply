...
const InventoryStats = memo<{ items: InventoryItem[] }>(({ items }) => {
  const stats = useMemo(() => {
    ...
    return { totalItems, belowParCount, expiringCount, outOfStockCount }
  }, [items])
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <Card className="apple-card group hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
        <CardContent className="p-6">
          ...
          <div className="text-3xl font-serif font-bold text-primary mb-2">{stats.totalItems}</div>
      ...
      <Card className="apple-card group hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
        <CardContent className="p-6">
          ...
          <div className="text-3xl font-serif font-bold text-destructive mb-2">{stats.belowParCount}</div>
      ...
      <Card className="apple-card group hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
        <CardContent className="p-6">
          ...
          <div className="text-3xl font-serif font-bold text-orange-600 mb-2">{stats.expiringCount}</div>
      ...
      <Card className="apple-card group hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
        <CardContent className="p-6">
          ...
          <div className="text-3xl font-serif font-bold text-destructive mb-2">{stats.outOfStockCount}</div>
...
export function InventoryClient({ items: initialItems, locations, userRole }: InventoryClientProps) {
  ...
  const alertStats = useMemo(() => {
    ...
    return { belowParCount, expiringCount, outOfStockCount }
  }, [items])
        ...
        if (editingItem && editingItem.id) {
          ...
          const { data, error } = await supabase
            ...
            .update({
              ...
              par_level: itemData.min_par_level,
    ...
    <div className="h-full">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
        ...
        <TabsContent value="inventory" className="space-y-8">
          ...
          {(alertStats.belowParCount > 0 || alertStats.expiringCount > 0 || alertStats.outOfStockCount > 0) && (
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {alertStats.expiringCount > 0 && (
                    <Badge className="text-sm font-medium px-3 py-1 rounded-xl bg-orange-500 text-white hover:bg-orange-600">
                      {alertStats.expiringCount} items expiring soon
                    </Badge>
                  )}
                  {alertStats.outOfStockCount > 0 && (
                    <Badge variant="destructive" className="text-sm font-medium px-3 py-1 rounded-xl">
                      {alertStats.outOfStockCount} items out of stock
                    </Badge>
                  )}
                </div>
              </CardContent>
