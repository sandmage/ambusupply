...
-- Daily Check Form System
-- Enables customizable vehicle inspection forms with admin review capabilities

-- Create daily check forms table (customizable form templates)
CREATE TABLE IF NOT EXISTS public.daily_check_forms (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT daily_check_forms_pkey PRIMARY KEY (id),
  CONSTRAINT daily_check_forms_vehicle_id_fkey FOREIGN KEY (vehicle_id) REFERENCES public.vehicles(id) ON DELETE CASCADE,
  CONSTRAINT daily_check_forms_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id),
  CONSTRAINT daily_check_forms_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id)
);

-- Create daily check submissions table (completed daily checks)
CREATE TABLE IF NOT EXISTS public.daily_check_submissions (
  ...
  shift_start timestamp with time zone,
  shift_end timestamp with time zone,
  responses jsonb NOT NULL, -- form responses with field IDs and values
  overall_status text NOT NULL CHECK (overall_status = ANY (ARRAY['pass'::text, 'fail'::text, 'needs_attention'::text])),
  issues_count integer DEFAULT 0,
  signature_data text, -- base64 encoded signature for mobile apps
  photos jsonb, -- array of photo URLs/references
  notes text,
  reviewed_by uuid,
  ...
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT daily_check_submissions_pkey PRIMARY KEY (id),
  CONSTRAINT daily_check_submissions_form_id_fkey FOREIGN KEY (form_id) REFERENCES public.daily_check_forms(id),
  CONSTRAINT daily_check_submissions_vehicle_id_fkey FOREIGN KEY (vehicle_id) REFERENCES public.vehicles(id),
  CONSTRAINT daily_check_submissions_submitted_by_fkey FOREIGN KEY (submitted_by) REFERENCES public.profiles(id),
  CONSTRAINT daily_check_submissions_reviewed_by_fkey FOREIGN KEY (reviewed_by) REFERENCES public.profiles(id),
  CONSTRAINT daily_check_submissions_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id)
);

-- Create daily check issues table (specific issues found during checks)
CREATE TABLE IF NOT EXISTS public.daily_check_issues (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  submission_id uuid NOT NULL,
  field_id text NOT NULL, -- references field in form checklist_items
  issue_type text NOT NULL CHECK (issue_type = ANY (ARRAY['missing'::text, 'damaged'::text, 'expired'::text, 'low_quantity'::text, 'malfunction'::text, 'other'::text])),
  ...
  organization_id uuid,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT daily_check_issues_pkey PRIMARY KEY (id),
  CONSTRAINT daily_check_issues_submission_id_fkey FOREIGN KEY (submission_id) REFERENCES public.daily_check_submissions(id) ON DELETE CASCADE,
  CONSTRAINT daily_check_issues_resolved_by_fkey FOREIGN KEY (resolved_by) REFERENCES public.profiles(id),
  CONSTRAINT daily_check_issues_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id)
);

-- Create indexes for daily check form tables
CREATE INDEX IF NOT EXISTS idx_daily_check_forms_vehicle ON public.daily_check_forms(vehicle_id);
...
CREATE INDEX IF NOT EXISTS idx_daily_check_submissions_date ON public.daily_check_submissions(submitted_at);
CREATE INDEX IF NOT EXISTS idx_daily_check_submissions_status ON public.daily_check_submissions(overall_status);
CREATE INDEX IF NOT EXISTS idx_daily_check_submissions_reviewed ON public.daily_check_submissions(reviewed_by, reviewed_at);
CREATE INDEX IF NOT EXISTS idx_daily_check_issues_submission ON public.daily_check_issues(submission_id);
CREATE INDEX IF NOT EXISTS idx_daily_check_issues_severity ON public.daily_check_issues(severity);
CREATE INDEX IF NOT EXISTS idx_daily_check_issues_resolved ON public.daily_check_issues(resolved_at);

-- Enable Row Level Security on daily check tables
...
-- Create RLS policies for daily check tables
DO $$ BEGIN
    CREATE POLICY "Users can view daily check forms" ON public.daily_check_forms
      FOR SELECT USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
    CREATE POLICY "Users can manage daily check forms" ON public.daily_check_forms
      FOR ALL USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
    CREATE POLICY "Users can view daily check submissions" ON public.daily_check_submissions
      FOR SELECT USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
    CREATE POLICY "Users can manage daily check submissions" ON public.daily_check_submissions
      FOR ALL USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
    CREATE POLICY "Users can view daily check issues" ON public.daily_check_issues
      FOR SELECT USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
    CREATE POLICY "Users can manage daily check issues" ON public.daily_check_issues
      FOR ALL USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

-- Insert sample daily check form template
INSERT INTO public.daily_check_forms (form_name, description, checklist_items, created_by, organization_id) 
SELECT 
  'Standard Vehicle Daily Check',
  'Comprehensive daily vehicle inspection checklist for ambulances',
  '[
    {"id": "exterior_lights", "label": "All exterior lights functional", "type": "checkbox", "category": "safety", "required": true},
    {"id": "interior_lights", "label": "All interior lights functional", "type": "checkbox", "category": "safety", "required": true},
    {"id": "siren_horn", "label": "Siren and horn operational", "type": "checkbox", "category": "safety", "required": true},
    {"id": "fuel_level", "label": "Fuel level", "type": "select", "options": ["Full", "3/4", "1/2", "1/4", "Low"], "category": "maintenance", "required": true},
    {"id": "oil_level", "label": "Oil level check", "type": "checkbox", "category": "maintenance", "required": true},
    {"id": "tire_condition", "label": "Tire condition and pressure", "type": "checkbox", "category": "maintenance", "required": true},
    {"id": "stretcher_operation", "label": "Stretcher operation", "type": "checkbox", "category": "equipment", "required": true},
    {"id": "suction_unit", "label": "Suction unit functional", "type": "checkbox", "category": "equipment", "required": true},
    {"id": "oxygen_levels", "label": "Oxygen tank levels", "type": "select", "options": ["Full", "3/4", "1/2", "1/4", "Empty"], "category": "equipment", "required": true},
    {"id": "defibrillator", "label": "Defibrillator/Monitor operational", "type": "checkbox", "category": "equipment", "required": true},
    {"id": "medication_check", "label": "Medication inventory complete", "type": "checkbox", "category": "inventory", "required": true},
    {"id": "supply_levels", "label": "Supply levels adequate", "type": "checkbox", "category": "inventory", "required": true},
    {"id": "cleanliness", "label": "Vehicle cleanliness", "type": "select", "options": ["Excellent", "Good", "Fair", "Poor"], "category": "maintenance", "required": true},
    {"id": "additional_notes", "label": "Additional notes or concerns", "type": "textarea", "category": "other", "required": false}
  ]'::jsonb,
  p.id,
  p.organization_id
...
-- Success message for daily check forms
SELECT 'Daily check form system schema added successfully!' as result;
